...
namespace Editor
{
    public static class UnityProjectJunctionTool
    {
        ...
        {
            var projectDirectoryInfo = new DirectoryInfo(Directory.GetCurrentDirectory());

            var junctionDirectory = $"{projectDirectoryInfo.Name}-Junction";
            var junctionPath = Path.Combine(projectDirectoryInfo.Parent.FullName, junctionDirectory);
            ...
            var junctionDirectoryInfo = Directory.CreateDirectory(junctionPath);

            var linkAssets = Path.Combine(junctionDirectoryInfo.FullName, "Assets");
            var linkProjectSettings = Path.Combine(junctionDirectoryInfo.FullName, "ProjectSettings");
            var linkPackages = Path.Combine(junctionDirectoryInfo.FullName, "Packages");

            var targetAssets = Path.Combine(projectDirectoryInfo.FullName, "Assets");
            var targetProjectSettings = Path.Combine(projectDirectoryInfo.FullName, "ProjectSettings");
            var targetPackages = Path.Combine(projectDirectoryInfo.FullName, "Packages");
        ...
        private static void LinkWinOS(string link, string target)
        {
            var command = $"/C mklink /J \"{link}\" \"{target}\"";
            Process.Start("cmd.exe", command);
        }

        private static void LinkMacLinOS(string link, string target)
        {
            var command = $"ln -s \"{target}\" \"{link}\"";
